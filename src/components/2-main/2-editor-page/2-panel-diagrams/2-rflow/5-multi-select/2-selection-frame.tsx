...
export function SelectionFrame() {
    ...
    function applyScaledBoxes(scaled: FlowBox[]) {
        ...
        const nodes = store.getState().getNodes().map(
            (node) => {
...
function handlePosition(corner: Corner): { left?: number; right?: number; top?: number; bottom?: number; transform: string; } {
    ...
    switch (corner) {
        case 'nw': return { left: 0, top: 0, transform: `translate(${shift}px, ${shift}px)` };
        case 'ne': return { right: 0, top: 0, transform: `translate(${-shift}px, ${shift}px)` };
        case 'sw': return { left: 0, bottom: 0, transform: `translate(${shift}px, ${-shift}px)` };
        default: return { right: 0, bottom: 0, transform: `translate(${-shift}px, ${-shift}px)` };
    }
}
