/**
 * Custom Mermaid flowchart parser + Dagre layout → React Flow nodes/edges.
 */
import { type Node, type Edge, MarkerType, Position } from "reactflow";
import mermaid from "mermaid";
import dagre from "dagre";
import { LAYOUT_DIRECTIONS, LAYOUT_SPACING, NODE_SHAPES } from "./8-constants";
...
export function parseMermaidCode(code: string): { nodes: MermaidNode[]; edges: MermaidEdge[]; subgraphs: SubgraphInfo[]; direction: string; } {
    ...
    lines.forEach(
        (line, lineIndex) => {
            ...
            if (!trimmedLine || trimmedLine.startsWith('subgraph') || trimmedLine === 'end' || trimmedLine.startsWith('%%')) {
                return;
            }
    ...
    subgraphs.forEach(
        (sg) => { debugLog(`- ${sg.id}: "${sg.title}" (parent: ${sg.parentId || "none"}, children: ${sg.childrenIds.join(", ") || "none"}, nodes: ${sg.nodes.length})`); }
    );

    debugLog("Final node assignments:");
    nodes.forEach(
        (node) => { debugLog(`- ${node.id}: "${node.label}" in subgraph ${node.subgraph || "none"} (shape: ${node.shape})`); }
    );

    debugLog("Final edges:");
    edges.forEach(
        (edge, index) => { debugLog(`- Edge ${index}: ${edge.source} -> ${edge.target} (label: "${edge.label}", type: "${edge.type}")`); }
    );
...
function layoutSubgraphs(nodes: MermaidNode[], edges: MermaidEdge[], subgraphs: SubgraphInfo[], direction: string): Map<string, SubgraphLayout> {
...
function recalculateParentSubgraphSizes(subgraphLayouts: Map<string, SubgraphLayout>, orderedSubgraphs: SubgraphInfo[], direction: string) {
...
function calculateConnectionWeights(nodes: MermaidNode[], edges: MermaidEdge[]): Map<string, Map<string, number>> {
...
function getParentNodesBottomBoundary(parentId: string, parentLayout: SubgraphLayout): number {
    let maxBottom = 0;

    // Check all nodes that belong directly to this parent subgraph
    parentLayout.nodes.forEach(
        (nodePos) => {
...
function validateNodeSpacing(nodePositions: Map<string, { x: number; y: number; width: number; height: number; }>, subgraphId: string): void {
...
function layoutMetaGraph(nodes: MermaidNode[], edges: MermaidEdge[], subgraphLayouts: Map<string, SubgraphLayout>, direction: string): {
    subgraphPositions: Map<string, { x: number; y: number; }>;
        ...
        (layout, id) => {
            if (!layout.parentId) {
                ...
                if (node) {
                    ...
                    debugLog(
                        `Positioned subgraph ${id} at (${node.x - layout.width / 2}, ${node.y - layout.height / 2
                        })`
                    );
                } else {
    ...
    const orderedForMeta = processSubgraphsInHierarchicalOrder(
        Array.from(new Set(
            Array.from(subgraphLayouts.keys()).map(
                (id) => ({
                    id,
...
function createReactFlowElements(
    ...
    nodes.forEach((node) => {
        ...
        switch (node.shape) {
            case "diamond": nodeStyle.borderRadius = "0px"; break;
            case "circle": nodeStyle.borderRadius = "50%"; break;
            case "stadium": nodeStyle.borderRadius = "30px"; break;
            case "round": nodeStyle.borderRadius = "15px"; break;
        }
...
function layoutGraph(nodes: MermaidNode[], edges: MermaidEdge[], subgraphs: SubgraphInfo[], direction: string): { nodes: Node[]; edges: Edge[]; } {
...
export async function debugConvertMermaid(mermaidCode: string): Promise<any> {
    ...
    const subgraphPositionsPlain = Object.fromEntries(Array.from(subgraphPositions.entries()));
    const standalonePositionsPlain = Object.fromEntries(Array.from(standalonePositions.entries()));
