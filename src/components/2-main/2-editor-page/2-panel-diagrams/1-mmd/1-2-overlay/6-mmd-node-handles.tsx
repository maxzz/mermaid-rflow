...
import { type DragLine, nodeIdFromPoint } from './5-mmd-edge-endpoints';
            ...
            ({ side }) => {
                ...
                return (
                    <button
                        ...
                        onPointerDown={(e) => onHandlePointerDown(e, side, { box: selected, shape, hostRef, setConnect })}
...
function onHandlePointerDown(
    ...
    function up(ev: PointerEvent | MouseEvent) {
        ...
        if (dx * dx + dy * dy < 64) {
            applyMmdPatchResult(addNode(mermaidSettings.source, { shape, fromId: box.id }));
            return;
        }
        const toId = nodeIdFromPoint(ev.clientX, ev.clientY, box.id);
        if (toId) {
            applyMmdPatchResult(connectNodes(mermaidSettings.source, box.id, toId));
        }
    }
